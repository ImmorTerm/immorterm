...
Most endpoints have no authentication. The trust boundary is the
...
The Session Bridge is the exception. Its deployment administrator credential
provisions short-lived installation credentials bound to one project and a
limited operation set. Normal directory, message, contract, and event-stream
operations require those scoped bearer credentials; host credentials cannot
acknowledge or reply for an agent. See [Session Bridge](/docs/session-bridge).

Consumers should preflight `GET /api/v1/bridge/identity`; it exposes only the
authenticated credential's non-secret claims. Agent acknowledgement and reply
use a separate opaque receipt bound to one message and one receiving daemon,
never an administrator or installation credential.

Legacy handlers report failures as `200` with an `{ "error": "..." }` body rather
...
The authenticated `/api/v1/bridge/*` surface provides stable project/session
discovery, durable correlated delivery, explicit agent acknowledgement, replies,
and a resumable event WebSocket. It uses ordinary HTTP status codes, including
`201`, `202`, `401`, `403`, `409`, `429`, and `503`. Hosted integrations use
`@immorterm/session-bridge` with scoped installation credentials; the deployment
credential is only an administrative provisioning secret. The complete contract
is documented in [Session Bridge](/docs/session-bridge).
The local UI control plane also exposes `GET /bridge/status`,
`PUT /bridge/settings`, and `POST /bridge/rotate-token`; these endpoints never
return the administrator bearer token.
