#!/usr/bin/env node
/**
 * postinstall — Resolve the correct platform binary for @immorterm/ai
 *
...
const BINARY_NAME = process.platform === "win32" ? "immorterm-ai.exe" : "immorterm-ai";

const PLATFORM_PACKAGES = {
	"darwin-arm64": "@immorterm/ai-darwin-arm64",
	"darwin-x64": "@immorterm/ai-darwin-x64",
	"linux-x64": "@immorterm/ai-linux-x64",
	"linux-arm64": "@immorterm/ai-linux-arm64",
};
...
function main() {
	...
	if (!pkg) {
		console.error(
			`@immorterm/ai: Unsupported platform ${platformKey}. ` +
	...
	try {
		...
		console.error(
			`@immorterm/ai: Platform package ${pkg} not found. ` +
	...
	if (!existsSync(sourcePath)) {
		console.error(`@immorterm/ai: Binary not found at ${sourcePath}`);
		process.exit(0);
	}
