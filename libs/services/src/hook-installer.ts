...
/**
 * Where the notify wrapper is deployed, canonical first.
 *
 * It is ImmorTerm's own script, referenced by ABSOLUTE path from the notify
 * hooks of EVERY vendor — so a Codex-only user was getting a `~/.claude/hooks/`
 * directory purely to hold our file. `~/.immorterm/hooks/` is the right home.
 *
 * Both are written during the transition, because hook configs generated by an
 * older installer still point at the old absolute path; deleting it would make
 * every one of those fire ENOENT on every event. New configs point at
 * NOTIFY_WRAPPER_TARGET. Drop the legacy entry once configs have been
 * regenerated everywhere — the installer rewrites them on each run, so one
 * pass per project is enough.
 */
const NOTIFY_WRAPPER_TARGETS = [
  path.join(process.env.HOME ?? '', '.immorterm', 'hooks', NOTIFY_WRAPPER_FILE),
  path.join(process.env.HOME ?? '', '.claude', 'hooks', NOTIFY_WRAPPER_FILE),
];
const NOTIFY_WRAPPER_TARGET = NOTIFY_WRAPPER_TARGETS[0];
...
if [ "$PLANS_ENFORCE" = "1" ]; then
  ...
  #   2. saved project-id (.immorterm/, legacy .claude/)
  #   3. folder basename
...
- USE THE PROJECT'S OWN DESIGN LANGUAGE. Look first for its identity — CLAUDE.md, a theme/tokens file, existing CSS custom properties, component styles, AND its real logo/wordmark + icons (reuse the actual assets, inlined as data URIs) — then author in THAT palette, type scale, components, and voice so the plan looks like it shipped from the project itself. If none exist, choose a considered palette grounded in the project (deliberate neutrals with a slight hue bias toward one accent). Never a generic, default-AI, or ImmorTerm-themed look — the body is the PROJECT's; ImmorTerm only frames it.
      ...
      PostToolUse: [
        ...
        // Paired with the PreToolUse:AskUserQuestion notify-attention below.
        {
          ...
          hooks: [
            {
              type: 'command',
              command: `node $HOME/.immorterm/hooks/immorterm-notify.mjs working`,
              timeout: 2,
            },
          ],
        },
      ...
      PreToolUse: [
        ...
        // Pair: PostToolUse:AskUserQuestion → notify working (resumes the pulse after answer).
        {
          ...
          hooks: [
            {
              type: 'command',
              command: `node $HOME/.immorterm/hooks/immorterm-notify.mjs attention`,
              timeout: 2,
            },
          ],
        },
      ...
      Stop: [
        {
          hooks: [
            ...
            {
              type: 'command',
              command: `node $HOME/.immorterm/hooks/immorterm-notify.mjs idle`,
              timeout: 2,
            },
          ],
        },
...
export function installMemoryHooks(
  ...
  try {
    ...
    // Deploy the cross-OS notify wrapper to the user's global ImmorTerm hooks
    // dir. Every vendor's notify hook references it by absolute path, so
    // without this step a fresh install would emit ENOENT noise on every hook
    // fire. Written to the legacy ~/.claude location too — configs generated by
    // an older installer still point there. Idempotent: each project install
    // rewrites the same static content. Same candidate-probe pattern as the
    // digest-llm-invoke shim above.
    try {
      ...
      if (wrapperSource) {
        for (const target of NOTIFY_WRAPPER_TARGETS) {
          const wrapperTargetDir = path.dirname(target);
          if (!fs.existsSync(wrapperTargetDir)) {
            fs.mkdirSync(wrapperTargetDir, { recursive: true });
          }
          fs.copyFileSync(wrapperSource, target);
