...
static RE_OUTPUT_HINT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\n*Read the output file to retrieve the result:[^\n]*").unwrap());
static RE_USER_QUERY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<user_query>(.*?)</user_query>\s*").unwrap());
static RE_SYS_REMINDER: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<system-reminder>.*?</system-reminder>\s*").unwrap());
static RE_IDE_OPENED: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<ide_opened_file>.*?</ide_opened_file>\s*").unwrap());
static RE_LOCAL_CAVEAT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<local-command-caveat>.*?</local-command-caveat>\s*").unwrap());
static RE_CMD_NAME: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<command-name>(.*?)</command-name>\s*").unwrap());
static RE_CMD_MESSAGE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<command-message>.*?</command-message>\s*").unwrap());
static RE_CMD_ARGS_EMPTY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"<command-args>\s*</command-args>\s*").unwrap());
static RE_CMD_ARGS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<command-args>(.*?)</command-args>\s*").unwrap());
static RE_LOCAL_STDOUT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?s)<local-command-stdout>.*?</local-command-stdout>\s*").unwrap());
                ...
                for block in arr {
                    if block.get("type").and_then(|t| t.as_str()) == Some("tool_result") {
                        ...
                        if let Some(&bidx) = pending.get(&tid) {
                            ...
                            if result_text.starts_with("<tool_use_error>")
                                && result_text.ends_with("</tool_use_error>")
                            {
                                result_text = result_text["<tool_use_error>".len()
                                    ..result_text.len() - "</tool_use_error>".len()]
                                    .to_string();
                            }
                            let is_err = block
                                .get("is_error")
...
static RE_BG_TASK: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[bg-task:\s*(.+?)\]").unwrap());
...
pub fn filter_empty_turns(turns: Vec<Turn>) -> Vec<Turn> {
    let mut out: Vec<Turn> = turns
        .into_iter()
        .filter(|t| {
            if !t.user_text.is_empty() {
                return true;
            }
            if !t.system_events.is_empty() {
                return true;
            }
