...
use conversation_adapters::{
    ConversationAdapter, detect_format_from_text,
    formats::{
...
fn codex_fixture_three_turns_with_tool_normalization() {
    ...
    assert_eq!(
        tc.input.get("command").and_then(|c| c.as_str()),
        Some("cd /tmp/test && ls")
    );
    ...
    assert_eq!(
        wtc.input.get("file_path").and_then(|v| v.as_str()),
        Some("/tmp/hello.txt")
    );
    assert_eq!(
        wtc.input.get("content").and_then(|v| v.as_str()),
        Some("hello world")
    );
    ...
    assert_eq!(
        etc.input.get("file_path").and_then(|v| v.as_str()),
        Some("/tmp/hello.txt")
    );
    assert_eq!(
        etc.input.get("old_string").and_then(|v| v.as_str()),
        Some("hello world")
    );
    assert_eq!(
        etc.input.get("new_string").and_then(|v| v.as_str()),
        Some("hello, world!")
    );
...
fn windsurf_fixture_two_turns() {
    ...
    assert!(
        turns[0]
            .blocks
            .iter()
            .any(|b| b.kind == BlockKind::Thinking)
    );
...
fn cline_fixture_two_turns() {
    ...
    assert!(
        turns[0].timestamp.starts_with("2025-11-28") || turns[0].timestamp.starts_with("2025-12"),
        "unexpected ts: {}",
        turns[0].timestamp
    );
}
...
fn aider_fixture_two_turns_markdown() {
    ...
    assert!(
        turns[0].blocks[0].text.contains("def fib(n):"),
        "expected fib() in turn 0 assistant text, got: {}",
        turns[0].blocks[0].text
    );
    assert!(
        turns[1].blocks[0].text.contains("Tests pass."),
        "expected 'Tests pass.' in turn 1 assistant text, got: {}",
        turns[1].blocks[0].text
    );
...
fn system_tags_stripped_and_reminders_absorbed() {
    ...
    for t in &turns {
        assert!(
            !t.user_text.contains("<system-reminder>"),
            "system-reminder should be stripped from user_text: {:?}",
            t.user_text
        );
        assert!(
            !t.user_text.contains("<user_query>"),
            "user_query wrapper should be stripped: {:?}",
            t.user_text
        );
    }
}
...
fn codex_0145_captures_mcp_calls_and_normalizes_exec() {
    ...
    let ok = calls
        .iter()
        .find(|c| c.name == "mcp__immorterm-memory__search_memory")
    ...
    let err = calls
        .iter()
        .find(|c| c.name == "mcp__immorterm__immorterm_screenshot")
